...
public interface AuctionRepository extends JpaRepository<Auction, Long>, AuctionQueryDslRepository {
    ...
    @Query("SELECT COUNT(a) > 0 FROM Auction a WHERE a.keyboard.member.id = :memberId AND a.keyboard.id = :keyboardId AND a.auctionStatus IN :statuses")
    boolean existsByMember_IdAndAuctionStatus(Long memberId, Long keyboardId,
